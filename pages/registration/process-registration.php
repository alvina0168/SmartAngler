...
if (isset($_FILES['payment_proof']) && $_FILES['payment_proof']['error'] === 0) {
    ...
    if (in_array($file_type, $allowed_types)) {
        ...
        if (!move_uploaded_file($_FILES['payment_proof']['tmp_name'], $upload_path)) {
            ...
            redirect(SITE_URL . '/pages/registration/register-tournament.php?id=' . $tournament_id);
        }
    } else {
...
if ($stmt->execute()) {
    ...
    $notif_stmt->bind_param("ii", $user_id, $tournament_id);
    $notif_stmt->execute();
    $notif_stmt->close();
    
    $_SESSION['success'] = 'Registration submitted successfully! Your registration is pending approval.';
    redirect(SITE_URL . '/pages/dashboard/myDashboard.php');
} else {
